# Performance


## Javascript

### Talks
#### How does JavaScript engine work under the hood
[JavaScript engines - how do they even? - Franziska Hinkelmann](https://youtu.be/p-iiEDtpy6I?si=yJD3tTpA9kcNEB_L)

[V8 internals for JS developers - Mathias Bynens](https://youtu.be/5UZzT_hgsl0?si=lcIJj4Y_XOYm3-64)

#### Critical Rendering Path
[Critical rendering path - Crash course on web performance - IIya Grigorik](https://youtu.be/PkOBnYxqj3k?si=MhUIW4-RvGRxJKMx)

[Optimizing the Critical Rendering Path in React - Alex Moldovan](https://youtu.be/25fkjIIk2_o?si=Rci2Cv8gQxzgkvyI)

#### How does browser work
[So how does the browser actually render a website - Ryan Seddon](https://youtu.be/SmE4OwHztCc?si=i8UvBy43tFqi1Lam)

[How browsers work - Kruno](https://youtu.be/0IsQqJ7pwhw?si=PTTcqIkHBJKd33W3)

#### Memory management
[Visualised guide to memory management in JavaScript - Kateryna Porshnieva](https://youtu.be/OG_AZnPokGw?si=Y_8GsGq4PptZhZRh)

[The Hitchhiker's Guide to All Things Memory in JS - Safia Abdalla](https://youtu.be/cpM5LYZdfUo?si=R1hvWkJlM5mCMdkR)

### Free courses
[Browser Rendering Optimization - Udacity](https://www.udacity.com/course/browser-rendering-optimization--ud860)